...
public class PickerPlaceholder extends IPicker.Impl implements IGui {
    ...
    public PickerPlaceholder() {
        WLabelScroll scroll = new WLabelScroll(7, 69, 8, 5, false).setLabels(LPlaceholder.getRecent())
            .setLsnrLeftClick(
                (i, v) -> notifyLsnr(
                    i.get(v)
                        .getLabel()));
        WTextField create = new WTextField(26, 7, 69).setListener(
            i -> i.setColor(
                i.getText()
                    .equals("") ? COLOR_TEXT_RED : COLOR_TEXT_WHITE));
        ...
        add(new WLine(36));
        add(
            new WButtonIcon(95, 7, 20, 20, BTN_YES, "common.confirm").setListener(
                i -> {
                    if (!create.getText()
                        .equals("")) callback.accept(new LPlaceholder(create.getText(), 1));
                }));
        add(scroll, create);
    }
}
