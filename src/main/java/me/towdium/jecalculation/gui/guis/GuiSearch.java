...
public class GuiSearch extends WContainer implements IGui {
    ...
    public GuiSearch() {
        ...
        add(new WPanel());
        add(new WSearch(i -> content.stream().filter(j -> j.one.getRep() == i).findFirst()
                                    .ifPresent(j -> JecaGui.displayGui(true, true, new GuiRecipe(j.two, j.three))),
                        new WTextField(25, 27, 90), labelScroll));
    ...
    public void refresh() {
        ...
        labelScroll
                .setLabels(content.stream().map(i -> i.one.getRep()).collect(Collectors.toCollection(ArrayList::new)));
    }
