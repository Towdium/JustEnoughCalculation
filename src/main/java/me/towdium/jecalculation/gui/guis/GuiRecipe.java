...
public class GuiRecipe extends WContainer implements IGui {
    ...
    void fromRecipe(Recipe r) {
        input.setLabel(Arrays.stream(r.getLabel(Recipe.IO.INPUT))
                             .map(ILabel::copy).collect(Collectors.toList()), 0);
        catalyst.setLabel(Arrays.stream(r.getLabel(Recipe.IO.CATALYST))
                                .map(ILabel::copy).collect(Collectors.toList()), 0);
        output.setLabel(Arrays.stream(r.getLabel(Recipe.IO.OUTPUT))
                              .map(ILabel::copy).collect(Collectors.toList()), 0);
    }
