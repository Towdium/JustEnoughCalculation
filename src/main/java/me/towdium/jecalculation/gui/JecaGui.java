...
import static net.minecraftforge.common.util.LogicalSidedProvider.WORKQUEUE;
...
public class JecaGui extends AbstractContainerScreen<JecaGui.JecaContainer> {
    ...
    public static void onFocus(ScreenEvent.MouseClickedEvent.Pre event) {
        if (!(event.getScreen() instanceof JecaGui)) return;
        JecaGui gui = getCurrent();
    ...
    public static void onMouse(ScreenEvent.MouseInputEvent event) {
        if (!(event.getScreen() instanceof JecaGui)) return;
        JecaGui gui = getCurrent();
        int xMouse = getMouseX();
        int yMouse = getMouseY();
    ...
    public static void displayGui(IGui root, @Nullable JecaGui parent) {
        ...
        if (s instanceof IRecipesGui || s instanceof ChatScreen) {
            ...
            BlockableEventLoop<?> executor = WORKQUEUE.get(LogicalSide.CLIENT);
    ...
    public static void onKey(InputEvent.KeyInputEvent event) {
        if (keyOpenGuiCraft.isDown()) JecaGui.openGuiCraft(null);
        if (keyOpenGuiMath.isDown()) JecaGui.openGuiMath(null);
    }
    ...
    public static void onTooltip(RenderTooltipEvent.Pre event) {
        if (Minecraft.getInstance().screen instanceof JecaGui) {
            JecaGui gui = getCurrent();
            boolean overlap = gui.root.onTooltip(gui, event.getX() - gui.leftPos,
                    event.getY() - gui.topPos, new ArrayList<>());
            if (overlap && !event.getItemStack().isEmpty()) event.setCanceled(true);
        }
    }
    ...
    public void drawFluid(Fluid f, int xPos, int yPos, int xSize, int ySize) {
        TextureAtlasSprite fluidTexture = Objects.requireNonNull(minecraft)
                ...
                .getSprite(f.getAttributes().getStillTexture());
    ...
    public void drawItemStack(int xPos, int yPos, ItemStack is, boolean centred, boolean hand) {
        ...
        //Utilities.drawItem(matrix, itemRenderer, is, xPos, yPos);
    ...
    public static class FontType {
        ...
        public int getTextWidth(String s) {
            return (int) Math.ceil(getCurrent().font.width(s) * (half ? 0.5f : 1));
        }

        public int getTextHeight() {
            return (int) Math.ceil(getCurrent().font.lineHeight * (half ? 0.5f : 1));
        }

        public String trimToWidth(String s, int i) {
            return getCurrent().font.plainSubstrByWidth(s, i * (half ? 2 : 1));
        }
    }
