...
public class WLabelGroup extends WContainer {
    ...
    public WLabelGroup(int xPos, int yPos, int column, int row, int xSize, int ySize, boolean multiple, boolean accept) {
        for (int j = 0; j < row; j++) {
            int r = j;
            IntStream.range(0, column).forEach(c -> {
                WLabel l = new WLabel(xPos + c * xSize, yPos + r * ySize, xSize, ySize, multiple, accept)
                        .setLsnrUpdate((i, v) -> {
