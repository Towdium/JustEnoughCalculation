...
public class ItemStackWrapper {
    ...
    public static boolean isTypeEqual(ItemStack one, ItemStack two) {
        return one != null && two != null && one.getItem() == two.getItem() &&
               one.getItemDamage() == two.getItemDamage();
    }
    ...
    public static String getDisplayAmount(ItemStack itemStack) {
        if (itemStack != null) {
            if (itemStack.getTagCompound() != null) {
                ...
                long l = itemStack.getTagCompound().hasKey("amount") ?
                         itemStack.getTagCompound().getLong("amount") :
                         -1;
                ...
                if (doChange) {
                    if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey("percentage")) {
                        itemStack.getTagCompound()
                                 .setInteger("percentage", itemStack.getTagCompound().getInteger("percentage") + 1);
                    } else {
                        itemStack.stackSize += 1;
                    }
                }
