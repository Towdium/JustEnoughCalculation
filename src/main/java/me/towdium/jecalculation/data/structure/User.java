    ...
    public static class Recipes {
        ...
        public List<Recipe> search(String group, ILabel label, Recipe.enumIoType type) {
            return search(records.get(group).map(Collection::stream).orElseGet(Stream::empty),
                          r -> IntStream.range(0, type.getSize()).anyMatch(
                                  i -> ILabel.MERGER.merge(label, r.getLabel(type)[i], true).isPresent()));
        }

        public List<Recipe> search(String group, String label, Recipe.enumIoType type) {
            return search(records.get(group).map(Collection::stream).orElseGet(Stream::empty),
                          r -> IntStream.range(0, type.getSize()).anyMatch(
                                  i -> Utilities.contains(r.getLabel(type)[i].getDisplayName(), label)));
        }

        public List<Recipe> search(ILabel label, Recipe.enumIoType type) {
            return search(records.stream().flatMap(i -> i.two.stream()), r -> IntStream.range(0, type.getSize())
                                                                                       .anyMatch(i -> ILabel.MERGER
                                                                                               .merge(label, r.getLabel(
                                                                                                       type)[i], true)
                                                                                               .isPresent()));
        }

        public List<Recipe> search(String label, Recipe.enumIoType type) {
            return search(records.stream().flatMap(i -> i.two.stream()), r -> IntStream.range(0, type.getSize())
                                                                                       .anyMatch(i -> Utilities
                                                                                               .contains(r.getLabel(
                                                                                                       type)[i].getDisplayName(),
                                                                                                         label)));
        }
