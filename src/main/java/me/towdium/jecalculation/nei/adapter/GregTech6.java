    ...
    public void handleRecipe(IRecipeHandler recipe, int index, List<Object[]> inputs, List<Object[]> outputs) {
        if (gtRecipeMap.isInstance(recipe)) {
            ...
            try {
                ...
                inputs.clear();
                inputs.addAll(Stream.concat(Arrays.stream(input), Arrays.stream(fluidInputs)).map(i -> new Object[] {i}).collect(Collectors.toList()));
                outputs.clear();
                outputs.addAll(Stream.concat(Arrays.stream(output), Arrays.stream(fluidOutputs)).map(o -> new Object[] {o}).collect(Collectors.toList()));
            } catch (NoSuchFieldException | IllegalAccessException e) {
                e.printStackTrace();
            }
//            for (int i = 0; i < inputs.size(); i++) {
//                Object[] objects = Arrays.stream(inputs.get(i)).map(o -> GregTech6.convertFluid((ItemStack) o)).toArray();
